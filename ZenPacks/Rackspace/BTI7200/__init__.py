...
import logging
import os
log = logging.getLogger('zen.ZenPacks.Rackspace.BTI7200')

import ZenPacks.Rackspace.BTI7200
...
OperatingSystem._relations += (
    ("serialPorts", ToManyCont(ToOne, "ZenPacks.Rackspace.BTI7200.serialPort", "os")),
    ("shelves", ToManyCont(ToOne, "ZenPacks.Rackspace.BTI7200.Shelf", "os")),
    ("slots", ToManyCont(ToOne, "ZenPacks.Rackspace.BTI7200.BSlot", "os")),
    )
...
class ZenPack(ZenPackBase):
    ...
    def install(self, app):
        ...
        # Install menus
        self.installMenuItems(app.zport.dmd, menuName='')
        ...
        self.createDeviceOrg(app.zport.dmd, organizer='/Network/BTI/7200')

        # Add modeler to organizer, if not existant
        self.addModeler(app.zport.dmd, organizer='/Network/BTI/7200', modelerNames=['snmp.bti.DeviceMap',  'snmp.bti.InterfaceMap', 'snmp.bti.SerialPortMap', 'snmp.bti.ShelfMap', 'snmp.bti.SlotMap'])

        # Rebuild relations
        self.rebuildRelations(app.zport.dmd)

        # Start daemon
        self.startDaemon(app.zport.dmd, daemonName='')
    ...
    def remove(self, app, leaveObjects=True):
        ...
        # Stop daemon
        self.stopDaemon(app.zport.dmd, daemonName='')

        # Remove menus
        self.removeMenuItems(app.zport.dmd, menuName='')

        if not leaveObjects:
            # Remove components based on classes extended by this package
            self.removeParts(app.zport.dmd, componentTypes='os', componentNames=[''])

            # Remove report organizers
            self.delReportOrg(app.zport.dmd, parent=None, organizer=None)

            # Remove modeler from organizer
            self.removeModeler(app.zport.dmd, organizer='/Network/BTI/7200', modelerNames=['snmp.bti.DeviceMap',  'snmp.bti.InterfaceMap', 'snmp.bti.SerialPortMap', 'snmp.bti.ShelfMap', 'snmp.bti.SlotMap'])

            # Remove organizer
            self.delDeviceOrg(app.zport.dmd, organizer='/Network/BTI/7200')
